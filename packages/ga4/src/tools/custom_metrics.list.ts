...
export const tool: ToolDefinition<Input, unknown> = {
  name: "ga4.custom_metrics.list",
  description: "List custom metrics on the configured GA4 property.",
  platform: "ga4",
  isWriteTool: false,
  inputSchema: Input,
  handler: async (input, ctx) => {
    ...
    const result = await client.admin("GET", `/properties/${property.property_id}/customMetrics`);
