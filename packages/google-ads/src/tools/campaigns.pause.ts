...
export const tool: ToolDefinition<Input, Output> = {
  name: "google_ads.campaigns.pause",
  description:
    "Pause a Google Ads campaign by mutating campaign.status to PAUSED. Reversible. Dry-run by default.",
  platform: "google_ads",
  isWriteTool: true,
  inputSchema: Input,
  handler: async (input, ctx) => {
    ...
    const decision = ctx.dryRunGate.evaluate({
      toolName: "google_ads.campaigns.pause",
      platform: "google_ads",
    ...
    if (decision.outcome === "allow_dry_run") {
      await audit(ctx, {
        tool: "google_ads.campaigns.pause",
        account: account.label,
        params: { campaign_id: input.campaign_id },
        dryRun: true,
        outcome: "allow_dry_run",
        resultSummary: `would pause ${resourceName} (was ${previousStatus ?? "unknown"})`,
      });
      return {
