...
export const tool: ToolDefinition<Input, Output> = {
  name: "linkedin.campaigns.resume",
  description: "Resume a paused LinkedIn campaign by PARTIAL_UPDATE to status=ACTIVE. Dry-run by default.",
  platform: "linkedin",
  isWriteTool: true,
  inputSchema: Input,
  handler: async (input, ctx) => {
    ...
    const decision = ctx.dryRunGate.evaluate({
      toolName: "linkedin.campaigns.resume",
      platform: "linkedin",
    ...
    if (decision.outcome === "allow_dry_run") {
      await audit(ctx, {
        tool: "linkedin.campaigns.resume",
        account: account.label,
        params: { campaign_id: input.campaign_id },
        dryRun: true,
        outcome: "allow_dry_run",
        resultSummary: `would resume campaign ${input.campaign_id} (was ${previousStatus ?? "unknown"})`,
      });
      return {
