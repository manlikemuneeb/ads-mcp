...
const Input = z
  .object({
    ...
    daily_budget_cents: z
      ...
      .positive()
      .optional()
      .describe("Daily budget in account-currency cents (e.g. 5000 = $50.00). Mutually exclusive with lifetime_budget_cents."),
    lifetime_budget_cents: z
      .number()
      .int()
      .positive()
      .optional()
      .describe("Lifetime budget in account-currency cents. Mutually exclusive with daily_budget_cents."),
  })
  .refine(
...
export const tool: ToolDefinition<Input, Output> = {
  name: "meta.campaigns.update_budget",
  description:
    "Update a Meta campaign's daily or lifetime budget. Pass exactly one of daily_budget_cents or lifetime_budget_cents in account-currency cents. Dry-run by default; high-risk write.",
  platform: "meta",
  isWriteTool: true,
  inputSchema: Input,
  handler: async (input, ctx) => {
    ...
    const decision = ctx.dryRunGate.evaluate({
      toolName: "meta.campaigns.update_budget",
      platform: "meta",
    ...
    if (input.lifetime_budget_cents !== undefined) body.lifetime_budget = input.lifetime_budget_cents;

    const params = {
      campaign_id: input.campaign_id,
      ...(input.daily_budget_cents !== undefined ? { daily_budget_cents: input.daily_budget_cents } : {}),
    ...
    if (decision.outcome === "allow_dry_run") {
      await ctx.auditLogger.log({
        tool: "meta.campaigns.update_budget",
        platform: "meta",
        account: account.label,
        params,
        dry_run: true,
        outcome: "allow_dry_run",
        result_summary: `would update budget on campaign ${input.campaign_id}`,
      });
      return {
        campaign_id: input.campaign_id,
    ...
    try {
      ...
      await ctx.auditLogger.log({
        ...
        result_summary: `budget updated on campaign ${input.campaign_id}`,
      });
      return {
        campaign_id: input.campaign_id,
        ...(previousDaily !== undefined ? { previous_daily_budget: previousDaily } : {}),
