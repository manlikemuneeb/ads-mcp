...
export const tool: ToolDefinition<Input, Output> = {
  name: "meta.campaigns.pause",
  description:
    "Pause a Meta campaign by setting status to PAUSED. Reversible via meta.campaigns.resume. Dry-run by default.",
  platform: "meta",
  isWriteTool: true,
  inputSchema: Input,
  handler: async (input, ctx) => {
    ...
    // Always read current state for audit + preview, even on dry-run.
    ...
    if (decision.outcome === "allow_dry_run") {
      await ctx.auditLogger.log({
        ...
        result_summary: `would pause campaign ${input.campaign_id} (was ${previousStatus ?? "unknown"})`,
      });
      return {
    ...
    // allow_live
    try {
      await client.post(`/${input.campaign_id}`, { status: "PAUSED" });
      await ctx.auditLogger.log({
        tool: "meta.campaigns.pause",
        platform: "meta",
        account: account.label,
